...
# ===== 路径 (AutoDL服务器路径) =====
DATA_DIR = "/root/autodl-tmp/eval_qwen2vl/训练数据"
EVAL_FILE = os.path.join(DATA_DIR, "vlm_eval_converted.jsonl")
REPORT_DIR = "/root/autodl-tmp/eval_qwen2vl"
os.makedirs(REPORT_DIR, exist_ok=True)

# 微调模型路径
LORA_MODEL_PATH = "/root/autodl-tmp/eval_qwen2vl/lora/sft"
BASE_MODEL_PATH = "/root/autodl-tmp/eval_qwen2vl/model/Qwen2-VL-2B-Instruct"
...
def evaluate_with_local_model(records, sample_size=50):
    """使用本地微调模型进行评估"""
    import torch
...
def main():
    print("=" * 60, flush=True)
    print("工单16 - VLM 微调评估脚本 (AutoDL GPU版)", flush=True)
    print("=" * 60, flush=True)
    ...
    finetuned_results = evaluate_with_local_model(records, sample_size=50)
    total_acc = sum(r["accuracy"] for r in finetuned_results) / max(len(finetuned_results), 1)

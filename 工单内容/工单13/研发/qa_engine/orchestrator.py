...
class RAGSystem:
        ...
        try:
            ...
            if VECTOR_STORE_BACKEND == "milvus":
                ...
                if self.use_cache:
                    ...
                    if loaded and self.vector_store.index is not None:
                        print(f"[RAGSystem] FAISS索引已加载 ({self.vector_store.index.ntotal} 条)")
            ...
            try:
                if hasattr(self, 'vector_store') and self.vector_store is not None:
                    if VECTOR_STORE_BACKEND == "milvus":
                        ...
                        store = self.vector_store
                        # FAISS: 检查索引是否有数据
                        if hasattr(store, 'index') and store.index is not None and store.index.ntotal > 0:
                            self._is_ready = True
                            print(f"[RAGSystem] FAISS已有数据 ({store.index.ntotal} 条), 标记为就绪")
            except Exception:
                pass
    ...
    def _load_pdf_internal(self, pdf_path: str, incremental: bool = False) -> dict:
        """内部PDF加载逻辑"""
        # 惰性初始化（防止Streamilt重渲染导致属性丢失）
        if not hasattr(self, 'pdf_parser') or self.pdf_parser is None:
            from pdf_parser.parser import PDFParser
            from pdf_parser.chunker import TextChunker
            self.pdf_parser = PDFParser()
            self.chunker = TextChunker(

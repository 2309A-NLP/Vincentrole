...
class Retriever:
    ...
    def retrieve(self, question: str, top_k: int = 5,
        ...
        try:
            ...
            if mode == "fulltext":
                ...
                results = self._retrieve_hybrid(expanded, lang, top_k, threshold)
            else:  # vector (default)
                results = self._retrieve_vector(expanded, lang, top_k, threshold)
    ...
    def _detect_table_type(self, text: str, lang: str) -> str:
        ...
        # 中文关键词检测器
        zh_detectors = [
            (["发行股数", "发行后总股本", "本次发行"], "股本结构"),
            (["募集资金", "投资项目", "募投", "补充流动资金"], "募投项目"),
            (["控制关系", "关联方", "持股比例", "关联关系"], "关联方"),
            (["收入", "营收", "销售"], "收入数据"),  # 基础类型，后面可能细化
            (["成本"], "成本数据"),
            (["技术标准"], "技术标准"),
        ]
        ...
        # 英文关键词检测器
        en_detectors = [
            (["shares", "offering", "equity"], "股本结构"),
            (["raised", "proceeds", "investment"], "募投项目"),
            (["related", "affiliate", "shareholder"], "关联方"),
            (["revenue", "income"], "收入数据"),
        ]
        
        # 根据语言选择检测器
        detectors = zh_detectors if lang == "zh" else en_detectors
        text_lower = text.lower()
        
        for keywords, table_type in detectors:
            for keyword in keywords:
                if lang == "zh":
                    if keyword in text:
                        # 细化收入类型
                        if table_type == "收入数据" and ("占比" in text or "比重" in text):
                            return "收入结构"
                        return table_type
                else:

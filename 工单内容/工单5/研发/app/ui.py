...
**支持语言：** 中文 / English（右上角切换）
        ...
        st.markdown("""
### Welcome to PDF Q&A System (Table-Optimized v4.0)
...
def main():
    """主入口"""
    st.set_page_config(
        page_title=config.UI_CONFIG["页面标题"] + " v4.0",
    ...
    lang = st.session_state.ui_lang
    if lang == "zh":
        title = f"{config.UI_CONFIG['页面图标']} {config.UI_CONFIG['页面标题']} v4.0"
    else:
        title = f"{config.UI_CONFIG['页面图标']} PDF Q&A System v4.0 (Table-Optimized)"
    st.title(title)
    ...
    # 显示系统版本信息
    st.caption(_("工单编号: 人工智能NLP-RAG-PDF文档的表格解析及检索优化 | v4.0",
                 "WO#: AI-NLP-RAG-PDF-Table-Optimization | v4.0"))
